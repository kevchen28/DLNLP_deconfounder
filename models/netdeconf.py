...
class GCN_DECONF(nn.Module):
    def __init__(self, nfeat, nhid, dropout, n_in=1, n_out=1, cuda=False):
        ...
        self.gc = nn.ModuleList([GCNConv(nfeat, nhid)] + [GCNConv(nhid, nhid) for _ in range(n_in - 1)])
        self.gc = self.gc.to(device)

        self.out_t00 = nn.ModuleList([nn.Linear(nhid, nhid) for _ in range(n_out)]).to(device)
        self.out_t10 = nn.ModuleList([nn.Linear(nhid, nhid) for _ in range(n_out)]).to(device)
